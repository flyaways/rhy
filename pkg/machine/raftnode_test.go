...
type cluster struct {
	...
	snapCount           uint64
	catchUpEntriesCount uint64
}

// newCluster creates a cluster of n nodes
func newCluster(n int, snapCount, ssCatchUpEntriesN uint64) *cluster {
	peers := make([]string, n)
	for i := range peers {
	...
	clus := &cluster{
		peers:               peers,
		commitC:             make([]<-chan *machine.Commit, len(peers)),
		errorC:              make([]<-chan error, len(peers)),
		proposeC:            make([]chan string, len(peers)),
		confChangeC:         make([]chan raftpb.ConfChange, len(peers)),
		snapshotTriggeredC:  make([]<-chan struct{}, len(peers)),
		snapCount:           snapCount,
		catchUpEntriesCount: ssCatchUpEntriesN,
	}
	...
	for i := range clus.peers {
		...
		fn, snapshotTriggeredC := func() (func() ([]byte, error), <-chan struct{}) {
		...
		clus.commitC[i], clus.errorC[i], _ = machine.NewRaftNode(i+1,
			clus.peers, false,
			snapCount, ssCatchUpEntriesN,
			fn, clus.proposeC[i], clus.confChangeC[i])
	}
...
func TestProposeOnCommit(t *testing.T) {
	clus := newCluster(3, 10000, 10000)
	defer clus.closeNoErrors(t)

	donec := make(chan struct{})
	for i := range clus.peers {
		// feedback for "n" committed entries, then update donec
		go func(pC chan<- string, cC <-chan *machine.Commit, eC <-chan error) {
			for n := 0; n < 100; n++ {
				c, ok := <-cC
				if !ok {
					pC = nil
...
func TestPutAndGetKeyValue(t *testing.T) {
	...
	var kvs *machine.KVStore
	getSnapshot := func() ([]byte, error) { return kvs.GetSnapshot() }
	commitC, errorC, snapshotterReady := machine.NewRaftNode(1, clusters, false,
		10000, 10000,
		getSnapshot, proposeC, confChangeC)

	kvs = machine.NewKVStore(<-snapshotterReady, proposeC, commitC, errorC)

	srv := httptest.NewServer(&machine.HttpKVAPI{
		Store:       kvs,
...
func TestAddNewNode(t *testing.T) {
	...
	machine.NewRaftNode(4, append(clus.peers, newNodeURL), true, 10000, 10000, nil, proposeC, confChangeC)
